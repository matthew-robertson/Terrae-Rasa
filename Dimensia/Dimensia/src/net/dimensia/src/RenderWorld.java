...
{	
	...
	public void renderToolSwing(World world)
	{
		if(!world.player.hasSwungTool || (world.player.inventory.getMainInventoryStack(world.player.selectedSlot) == null) || !(Item.itemsList[world.player.inventory.getMainInventoryStack(world.player.selectedSlot).getItemID()] instanceof ItemTool))
		{
			return;
		}
		
		GL11.glTranslatef(world.player.x + 9, world.player.y + 9, 0);
		...
		int h = size;
		int w = size;        
		float angle = world.player.rotateAngle / 57.5f;
        ...
        textures[world.player.inventory.getMainInventoryStack(world.player.selectedSlot).getItemID()].bind();        
        
        if(!world.player.isSwingingRight)
        {
        	GL11.glRotatef(world.player.rotateAngle, 0, 0, 1);
            t.startDrawingQuads();
        ...
        {
        	GL11.glRotatef(world.player.rotateAngle + 90.0f, 0, 0, 1);
