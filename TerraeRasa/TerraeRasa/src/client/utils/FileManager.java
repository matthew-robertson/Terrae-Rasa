...
{		
	...
	{
		try {
			SPGameEngine.terraeRasa.gameEngine.requestClose();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
			...
			throws IOException, ClassNotFoundException, URISyntaxException
	{
		...
		String xml = new SaveHelper().getFileXML(fileName, false);
		return xml;
	}
	...
	{
		verifyDirectoriesExist();
		new SaveHelper().saveFile("/settings.xml", settings);
	}	
	...
	{
		return (Settings)new SaveHelper().loadFile("/settings.xml");
	}
}
