...
public class MainMenu extends Render
{
	...
	public MainMenu(Settings settings)
	{
		...
		for(int i = 0; i < 5; i++)
		{
		    ...
		    star.velocity = new Vector2(random.nextFloat() * 3, random.nextFloat() * 3);
		    star.acceleration = new Vector2(random.nextFloat() / 20, random.nextFloat() / 20);
			stars.add(star);
		}
		...
		for(int i = 0; i < 7; i++)
		{
		    ...
		    star.zrot = random.nextInt(100);
		    star.velocity = new Vector2(0, 0);
		    star.acceleration = new Vector2(0, 0);
	...
	public void render(Settings settings)
	{
		...
		if(flaggedForWorldGen)
		{
			...
			//TODO [minor-questionable-bugfix]: Make this work without a hacky access into the server files.
			String name = worldName.getText();
			EnumWorldSize worldSize = EnumWorldSize.getSize(this.worldSize.getValue());
			EnumWorldDifficulty difficulty = EnumWorldDifficulty.getDifficulty(worldMode.getValue());
			WorldServerEarth world = new WorldGenEarth().generate(new WorldServerEarth(name, worldSize.getWidth(), worldSize.getHeight(), difficulty), 0, worldSize.getWidth(), 0, worldSize.getHeight());
		...
		for(Particle star : stars)
		{
			if(!settingsMenu.open)
			{
				...
				for(int i = 0; i < 6; i++)
				{
					...
					starTrail.r = color.getColors()[0];
					starTrail.g = color.getColors()[1];
				    starTrail.b = color.getColors()[2];
				    starTrail.a = 1;
				    ...
				    starTrail.velocity = new Vector2(star.velocity.getX() * -0.35F, star.velocity.getY() * -0.35F);
				    starTrail.acceleration = new Vector2(star.acceleration.getX() * -0.35F, star.acceleration.getY() * -0.35F);
			...
			if(star.x > Display.getWidth() * 0.6 || star.y > Display.getHeight() * 0.6)
			{
			    ...
			    star.velocity = new Vector2(velocityModifier * random.nextFloat() * 3 + 1, velocityModifier * random.nextFloat() * 4);
			    star.acceleration = new Vector2(random.nextFloat() / 15, random.nextFloat() / 15);
			}
		}	
	}
	...
	private void startMPGame()
	{
		...
		SPGameEngine.startMPGame();
	}
	
	private void requestGameConnection()
	{
		try {
			SPGameEngine.terraeRasa.gameEngine.setActivePlayerName(selectedPlayerName);
			
			String ip = (selectedServer.getIP()); 
			int port = Integer.parseInt(selectedServer.getPort());

			ServerInformationHelper connect = new ServerInformationHelper();
...
//			}
			...
			ClientConnectionThread thread = new ClientConnectionThread(socket, SPGameEngine.terraeRasa.gameEngine.getEngineLock(), os, is);
			thread.start();
			SPGameEngine.registerClientThread(thread);
			startMPGame();
		} catch (IOException e) {
			e.printStackTrace();
		}
...
//		System.out.println("Starting local server.");
		SPGameEngine.terraeRasa.gameEngine.startMPServer(selectedWorldName);
		
		while(!TerraeRasa.isMPServerRunning)
		{
			try {
				Thread.sleep(2500);
	...
	/**
	 * A quick and dirty fix for a load screen when the player decides to generate a world. 
