...
public class RenderEntities extends Render
{
	...
	{		
		...
		for(int i = 0; i < scaled_points.length; i++)
		{
			x_points[i] =  (scaled_points[i].getX() * Math.cos(angle)) - 
					(scaled_points[i].getY() * Math.sin(angle));
			y_points[i] = (scaled_points[i].getX() * Math.sin(angle)) + 
	 ...
	 */
	public void renderWorldEntityList(WorldClientEarth world)
	{
	 ...
	 */
	public void renderNPCs(WorldClientEarth world){
	 ...
	 */
	public void renderWorldProjectileList(WorldClientEarth world)
	{
	 ...
	 */
	public void renderItems(WorldClientEarth world)
	{
	...
	public void renderTemperaryText(World world)
	{
		for(int i = 0; i < world.temporaryText.size(); i++)
		{
			...
			renderText(temp.message, temp.x, temp.y, temp.color.getColors());		
		}
		GL11.glColor4f(1, 1, 1, 1);
	}
