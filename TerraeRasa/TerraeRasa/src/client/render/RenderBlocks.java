...
{
	public void renderBackwall(WorldClientEarth world, EntityPlayer player)
	{
		...
		ChunkClient[] chunks = ChunkUtils.getRequiredChunks(world, player);
		TERRAIN_GROUND.bind();
		...
		for(int i = 0; i < chunks.length; i++) //Loop each X Chunk
		{
			...
			{	
				...
				{						
					ClientMinimalBlock block = chunks[i].backWalls[k][l];
					int x1 = k + baseX;
					int y1 = l;
	...
	public void render(WorldClientEarth world, EntityPlayer player) 
	{
		adjustCameraToLastPosition();
		final int xsize = (int)(Display.getWidth() / 11.1);
		final int ysize = (int)(Display.getHeight() / 11.1) + 6;
		...
		for(int i = 0; i < chunks.length; i++) //Loop each X Chunk
		{
			...
			{	
				for(int l = yoff; l < yoff + loopY; l++) //y
				{						
					ClientMinimalBlock block = chunks[i].blocks[k][l];
					int x1 = k + baseX;
					int y1 = l;
