...
public class PassiveBonusFactory 
{
	...
	/**
	 * Gets the active passive bonuses for an armor type. This is based on the total number of pieces required.
	 * @param inventory the player's inventory
	 * @param type the EnumArmor tier to get PassiveBonuses for
	 * @return a PassiveBonus[] of all the activate passive bonuses for a given tier of gear
	 */
	private static PassiveBonus[] getBonusesByArmorType(InventoryPlayer inventory, EnumArmor type)
	{
		...
		//Get the passive bonuses based on piecesEquipped
	 ...
	 * different passive bonuses such as the gold2Piece, and silver2Piece - giving the effect of both.
	 * @param inventory the player's inventory
	...
	public static PassiveBonusContainer getPassiveBonuses(InventoryPlayer inventory)
	{
		...
		{
			PassiveBonus[] setBonuses = getBonusesByArmorType(inventory, armorType);
			for(PassiveBonus bonus : setBonuses)
			{
