...
public class EntityProjectile extends EntityParticle 
{
	...
	public void moveProjectile(World world){
		...
		if (isOnGround(world) || (canMoveRight(world) == 0 && velocity.getX() > 0) || (canMoveLeft(world) == 0 && velocity.getX() < 0) || canMoveUp(world) == 0){
			active = false;
			setIsFriendly(false);
			setIsHostile(false);
		}		
	...
	public EntityProjectile setProjectileSpriteIndex(int x, int y) {
		iconX = x;
		iconY = y;
		return this;
	}
