...
import server.utils.ChestLootGenerator;
import server.utils.SaveHelper;
import server.utils.SpawnManager;
 ...
 * @since       1.0
 */
public class WorldServerEarth extends WorldServer
{
	 ...
	 */
	public WorldServerEarth(String universeName, int width, int height, EnumWorldDifficulty difficulty)
	{
		super(universeName);
		setChunks(new ConcurrentHashMap<String, Chunk>(10));
		this.width = width;
	...
	{
		Entity entity = (Entity) entitiesByID.get(""+id);
		...
		try {
			for(int i = 0; i < height - 1; i++)
			{
				...
				{	
					...
					player.y = (i * 6) - 18;			
					player.grantImmunityTicks(6 * MPGameLoop.TICKS_PER_SECOND);
					return player;
				}			
			}
		} catch (Exception e) {
			...
			throws FileNotFoundException, IOException, ClassNotFoundException
	{
		...
		SavableWorld savable = null;
		//TODO [minor-badness]: This is likely something that should be refined later
		if(TerraeRasa.IS_MP_LAUNCH)
		{
			savable = (SavableWorld)(manager.loadFile("/" + universeName + "/" + dir + "/worlddata.xml"));
		}
		else
		{
			savable = (SavableWorld)(manager.loadFile("/World Saves/" + universeName + "/" + dir + "/worlddata.xml"));
		}
...
//				int originalSize = stack.getStack().getStackSize();
				ItemStack remainingStack = player.inventory.pickUpItemStack(player, stack.getStack()); //if so try to pick it up
...
//		}
	...
	private void updateWeather(ServerUpdate update)
	{
        ...
        while (keys.hasMoreElements()) 
        {
            ...
            else
            {
    			...
    			if(biome != null && biome.getBiomeID() == Biome.arctic.getBiomeID()) //if the biome is arctic
    			{
    				if(random.nextInt(150000) == 0) //and a random chance is met
    				{
    					//cause weather!
						chunk.weather = new WeatherSnow(chunk, averageSkyHeight); 
	...
	private void performEnemyToolHittests(ServerUpdate update, EntityPlayer player) 
	{
		try {
			...
			Vector2[] scaled_points = new Vector2[x_bounds.length];
			for(int i = 0; i < scaled_points.length; i++)
			{
				scaled_points[i] = new Vector2((float)(size * x_bounds[i]), (float)(size * ((float)y_bounds[i])) - (float)size );
			}
			...
			for(int i = 0; i < scaled_points.length; i++)
			{
				x_points[i] =  player.x + const_ + (scaled_points[i].getX() * Math.cos(angle)) - 
						(scaled_points[i].getY() * Math.sin(angle));
				y_points[i] = player.y + const_ + (scaled_points[i].getX() * Math.sin(angle)) + 
						( scaled_points[i].getY() * Math.cos(angle));
			}
...
//		long time = System.currentTimeMillis();
		...
		final int loadDistanceHorizontally = ((2 * Chunk.getChunkWidth()) + MPGameEngine.terraeRasa.getSettings().loadDistance * Chunk.getChunkWidth()) - 1;
...
//		System.out.println("Chunk_Check = " + (System.currentTimeMillis() - time));
	 ...
	 */
	public void launchProjectile( int angle, EntityProjectile projectile, double x, double y)
	{
