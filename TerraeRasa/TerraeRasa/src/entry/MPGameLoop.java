 ...
 * @since       1.0
 */
public class MPGameLoop extends Thread
{
	...
	private WorldServerEarth world;
	 ...
	 */
	public MPGameLoop(String universeName)
	{
	...
	public void run()
	{
		try {
			...
			MPGameEngine.canAcceptConnections = true;
			
		    while(!MPGameEngine.done) //Main Game Loop
		    {
		        loops = 0;
...
//		        	}	
		        	...
		        	MPGameEngine.processCloseRequests();
		        	...
		        	{
		        		Commands.processConsoleCommand(MPGameEngine.terraeRasa.getSettings(), update, players, world, this, val);
		        	}
...
//		        			> TODO: ?Create a new update object and defer the compression of that object?
		        	...
		        	MPGameEngine.addWorldUpdate(update);
...
//	        	System.out.println(end - start);
			...
			System.out.println(">>>>> Fatal Error broke the MP Game Loop<<<<<");
			e.printStackTrace();			
	...
	public WorldServerEarth getWorld() {
		return world;
	}
	...
	//TODO: [CRITICAL_BUG]: fix a null player here crashing the server
...
//		manager.loadWorld("Earth", worldName);
	...
	private void loadWorld()
	{
		if(TerraeRasa.IS_MP_LAUNCH)
		{
			...
			{
				try {
					this.world = (WorldServerEarth) manager.loadWorld("/Earth", universeName);
			...
			else
			{
				this.world = manager.generateAndSaveWorld(universeName, 
						MPGameEngine.terraeRasa.getSettings().worldSize, 
						MPGameEngine.terraeRasa.getSettings().worldDifficulty);
			}
		...
		{
			File file = new File(TerraeRasa.getBasePath() + "/World Saves/" + universeName);
			FileManager manager = new FileManager();
			if(file.exists())
			{
				try {
